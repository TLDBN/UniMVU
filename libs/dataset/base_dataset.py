...
import json
import time
from typing import  Any, Dict, Optional, Sequence, List, Tuple, Union, Set
import ipdb
...
from .video_loading_utils import get_transforms_video, process_video_with_decord, process_video_with_tv, process_video_with_safe_video_reader
...
class LazySupervisedVideoDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.declared_modalities = list(data_args.modalities) if data_args.modalities is not None else None
        if self.declared_modalities is None:
            base_modalities = ["video"]
            if self.fast_feat_type:
                base_modalities.append(self.fast_feat_type)
            self.declared_modalities = base_modalities
        self._invalid_indices: Set[int] = set()
        self._filtered_counter: int = 0
        ...
        if self.data_sample_ratio is not None:
            ...
            if isinstance(slow_path_mapping_path, list):
                if len(self.data_sample_ratio) == 1:
                    self.data_sample_ratio = self.data_sample_ratio * len(slow_path_mapping_path)
                assert len(self.data_sample_ratio) == len(slow_path_mapping_path)
            else:
                assert len(self.data_sample_ratio) == 1
        ...
        if isinstance(anno_path, list):
            ...
            if self.use_slow or self.use_slow_feat: # use raw video frames
                ...
                for i, (curr_anno_path, curr_fast_path_mapping_path, curr_data_root, curr_slow_path_mapping_path, curr_slow_path_data_root) in \
                    ...
                    if self.data_sample_ratio is not None:
                        ...
                        curr_selected_len = max(1, int(curr_ratio * len(curr_anno)))
            ...
            if self.data_sample_ratio is not None:
                assert len(self.data_sample_ratio) == 1
                # handle the special case for the data sample ratio is 0, we need to sample at least one sample
                selected_len = max(1, int(self.data_sample_ratio[0] * len(all_filtered_anno)))
                all_filtered_anno = all_filtered_anno[:selected_len]
                print(anno_path, 'sample ratio:', self.data_sample_ratio[0], 'remaining len:', len(all_filtered_anno))
    ...
    def _get_item(self, i):
        if i in self._invalid_indices:
            if len(self._invalid_indices) == len(self.list_data_dict):
                raise RuntimeError("All samples filtered out; no valid supervision remaining.")
            next_idx = (i + 1) % len(self.list_data_dict)
            return self._get_item(next_idx)
        ...
        if 'video' in sources[0]:
            ...
            if self.use_fast: # This is for a special test which directly load the video frames as the video feature
                ...
                if self.fast_feat_type == 'languagebind' or self.fast_feat_type == 'languagebind_14x14' or self.fast_feat_type == 'internvideo2' or self.fast_feat_type == 'siglip' or self.fast_feat_type == 'dense_video':
            ...
            if self.use_slow: 
                ...
                if os.path.isdir(video_file_path): # is bunch of image
                    ...
                    # TODO: Change the modality indicator to video feats, this should be done in the Dataloader
                    image = [(image, 100, ["video", self.fast_feat_type])]
                    ...
                    try:
                        ...
                        elif self.video_loading_backbone == 'safe_video_reader':
                            video, video_time, frame_time, num_frames_to_sample = process_video_with_safe_video_reader(video_file_path, self.data_args) # (32, 360, 480, 3)
                        else:
                            # ipdb.set_trace()
                        ...
                        # TODO: Change the modality indicator to video feats, this should be done in the Dataloader
                        image = [(image, video[0].size, ["video", self.fast_feat_type])]
                        # ipdb> video[0].size
                        # 2764800
                        # ipdb> video[0].shape
                        # (720, 1280, 3)
        ...
        if isinstance(i, int):
            ...
            if not torch.any(data_dict["labels"].ne(IGNORE_INDEX)):
                newly_invalid = i not in self._invalid_indices
                self._invalid_indices.add(i)
                if newly_invalid:
                    self._filtered_counter += 1
                    self._log_filtered_sample(i, self.list_data_dict[i])
                if len(self._invalid_indices) == len(self.list_data_dict):
                    raise RuntimeError("All samples filtered out; no valid supervision remaining.")
                next_idx = (i + 1) % len(self.list_data_dict)
                return self._get_item(next_idx)
    ...
    def _log_filtered_sample(self, index: int, metadata: Dict[str, Any]) -> None:
        sample_hint = (
            metadata.get("video")
            or metadata.get("id")
        ...
        message = (
            f"[LazySupervisedVideoDataset] Filtered sample idx={index} "
            f"(id={sample_hint}) due to missing supervised tokens. "
            f"Total filtered={self._filtered_counter}."
        )
        if self._filtered_counter <= 10 or self._filtered_counter % 50 == 0:
            print(message)
        logger.warning(message)
        if wandb and wandb.run is not None:
            try:
                wandb.log({"data/filtered_no_supervision": self._filtered_counter}, commit=False)
            except Exception as exc:  # pragma: no cover
                logger.debug("Unable to log filtered sample to wandb: %s", exc)
...
class DataCollatorForSupervisedVideoDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        valid_token_counts = labels.ne(IGNORE_INDEX).sum(dim=1)
        if (valid_token_counts == 0).any():
            zero_mask = valid_token_counts == 0
            zero_indices = zero_mask.nonzero(as_tuple=False).flatten().tolist()
            sample_hints = []
            for idx in zero_indices:
                meta = instances[idx].get("video_meta") or instances[idx]
                sample_hints.append(
                    meta.get("video")
                    or meta.get("id")
            ...
            msg = (
                ...
                f"Batch contains {len(zero_indices)} sample(s) with zero supervised tokens. "
                f"Examples: {sample_hints[:3]}"
            )
            print(msg)
            logger.warning(msg)
            if wandb and wandb.run is not None:
                try:
                    wandb.log(
                        {
                            "data/batch_zero_supervision": len(zero_indices),
                            "data/batch_zero_supervision_examples": ", ".join(sample_hints[:5]),
                        },
                        commit=False,
                    )
                except Exception as exc:  # pragma: no cover
                    logger.debug("Unable to log zero-supervision batch to wandb: %s", exc)
        ...
        if 'video_feat' in instances[0]:
            ...
            if all(x is not None and x.shape == video_feats[0].shape for x in video_feats):
                ...
                # Fall back to per-sample storage when spatial/channel shapes do not match.
                reference = video_feats[0]
                can_pad = (
                    reference is not None
                    and all(
                        (feat is None)
                        or (
                            feat.shape[0] == reference.shape[0]
                            and feat.shape[2:] == reference.shape[2:]
                        )
                ...
                if can_pad:
                    ...
                    C, _, H, W = reference.shape
                    padded_tensor = torch.zeros([len(instances), C, max_len, H, W], dtype=reference.dtype, device=reference.device)
        ...
        if 'second_feat' in instances[0]:
            ...
            if all(x is not None and x.shape == second_feats[0].shape for x in second_feats):
                ...
                valid_map: Dict[Tuple[int, int, int], List[int]] = {}
                for idx, feat in enumerate(second_feats):
                    if feat is None:
                        continue
                    shape_key = (feat.shape[1], feat.shape[2], feat.shape[3])
                    valid_map.setdefault(shape_key, []).append(idx)
                if not valid_map:
                    raise ValueError("Encountered empty secondary feature list while collating batch.")
                primary_shape, primary_indices = max(valid_map.items(), key=lambda kv: len(kv[1]))
                if len(valid_map) > 1:
                    print(f"[DataCollator] Mixed modality shapes detected in second_feats; resampling to shape {primary_shape}.")
                    import random
                    for idx, feat in enumerate(second_feats):
                        if feat is None:
                            continue
                        if (feat.shape[1], feat.shape[2], feat.shape[3]) != primary_shape:
                            replacement_idx = random.choice(primary_indices)
                            second_feats[idx] = second_feats[replacement_idx].clone()
                C = second_feats[primary_indices[0]].shape[0]
                max_t, max_h, max_w = primary_shape
                dtype = second_feats[primary_indices[0]].dtype
                device = second_feats[primary_indices[0]].device
                padded_tensor = torch.zeros(len(instances), C, max_t, max_h, max_w, dtype=dtype, device=device)
                for i, feat in enumerate(second_feats):
                    if feat is None:
                        continue
                    padded_tensor[i, :, :feat.shape[1], :feat.shape[2], :feat.shape[3]] = feat
