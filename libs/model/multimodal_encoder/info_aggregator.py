# Temporal aggregation modules for UniMVU.
...
class TemporalAggregator(nn.Module):
    """
    Temporal aggregation backbone used by UniMVU for fast-path feature encoding.
    """
    def __init__(
        self,
    ...
    def forward(self, x, frame_num, 
                ...
                slow_feats=None):
        # ipdb.set_trace()
        # x: other modality feature in video sequence with shape (B, T(max_length_in_batch), H, W, C)
        ...
        for curr_feat, curr_query, curr_len in zip(x, slow_feats, frame_num):
            ...
            output = output.view(-1, output.shape[-1]) #(1, T_q, C, D) -> (T_q * C, D)
            output = output.unsqueeze(dim=0) # (T_q * C, D) -> (1, T_q * C, D)
            all_features.append(output)
        
        # ipdb.set_trace()
        output = torch.cat(all_features, dim=0) # (B, T_q * C, D)
