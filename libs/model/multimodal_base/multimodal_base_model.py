"""Core multimodal building blocks for UniMVU."""
...
from libs.model.multimodal_encoder.builder import build_video_tower, build_temporal_aggregator, build_vision_tower
from libs.model.multimodal_projector.builder import build_vision_projector
...
class LMMMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_video_tower"):
            ...
            if config.mm_video_tower is not None:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        #### The Fast-path(other modality like audio, image(depth map...)) Init ###
        ...
        if hasattr(model_args, 'video_tower'):
            if self.get_video_tower() is None and model_args.video_tower is not None:
        ...
        ### The Slow-path(RGB Images video) Init### 
        if hasattr(model_args, 'vision_tower') and self.get_vision_tower() is None:
            ...
            # Load the whoel pretrained vision encoder
            pretrain_vision_modules = model_args.pretrain_vision_modules
            ...
            if pretrain_vision_modules is not None:
                ...
                incompatible_keys = self.vision_tower.load_state_dict(get_weight(whole_vision_weights, "vision_tower"), strict=False)
                print(f"Loaded vision tower weights from {pretrain_vision_modules}. Incompatible keys: {incompatible_keys}")
                incompatible_keys = self.mm_projector.load_state_dict(get_weight(whole_vision_weights, "mm_projector"), strict=False)
...
class LMMMetaForCausalLM(ABC):
    ...
    def get_2dPool(self, image_feature, stride=2):
        ...
        return image_feature, scaled_shape

    def image_shape_alignment(self, video_3d_feature, scaled_shape):
        height_3d, weight_3d = video_3d_feature.shape[1:3]
        if scaled_shape != [height_3d, weight_3d]:
            video_3d_feature = video_3d_feature.permute(0, 3, 1, 2).contiguous()
            video_3d_feature = nn.functional.interpolate(video_3d_feature, size=scaled_shape, mode='bilinear')
        video_3d_feature = video_3d_feature.permute(0, 2, 3, 1).contiguous()
        return video_3d_feature
            ...
            if return_feat_before_mlp:
                ...
                for idx, (image_feat, image_feat_before_mlp) in enumerate(zip(encoded_image_features, encoded_image_features_before_mlp)): # [torch.Size([32, 196, 896]), torch.Size([32, 196, 896])]
                    if idx in video_idx_in_batch: # video_idx_in_batch: [0]
                        image_feat, scaled_shape = self.get_2dPool(image_feat)
                        image_features.append(image_feat) ## Video call this torch.Size([32, 169, 896])
                        image_feat_before_mlp, scaled_shape = self.get_2dPool(image_feat_before_mlp)
                        image_features_before_mlp.append(image_feat_before_mlp) ## Video call this torch.Size([32, 169, 896])
                    else:
                        image_features.append(image_feat)
                        image_features_before_mlp.append(image_feat_before_mlp)
                
                return image_features, video_idx_in_batch, image_features_before_mlp, scaled_shape
            else:
                ...
                ### For chunked video frames
                # encoded_image_features: torch.Size([512, 729, 896]) 
                # 729 = 27 * 27 => the feature of one patches
                # 512 = 32 * 16 => total_frames * image in each frame chunk
                ### For selected video frames
                # torch.Size([32, 729, 896]) => 32 is the number of frames in a video
                encoded_image_features = torch.split(encoded_image_features, split_sizes) 
                image_features = []
    ...
    def _pad_and_stack_new_sequences(self, input_embeds, labels, attention_mask, position_ids):
        """
        Pad variable-length `input_embeds` and `labels` to a common length and stack into batch tensors.

        Returns:
            input_embeds_tensor: torch.Tensor of shape [B, L, D]
            labels_padded: torch.Tensor of shape [B, L]
            attention_mask_padded: torch.Tensor of shape [B, L]
            position_ids_padded: torch.Tensor of shape [B, L]
        """
        ...
        for i, (cur_embed, cur_labels) in enumerate(zip(input_embeds, labels)):
            cur_len = cur_embed.shape[0]
            if getattr(self.config, 'tokenizer_padding_side', 'right') == "left":
                input_embeds_padded.append(torch.cat((
                ...
                if cur_len > 0:
                    ...
                    position_ids_padded[i, -cur_len:] = torch.arange(0, cur_len, dtype=position_ids_padded.dtype, device=position_ids_padded.device)
            else:
                input_embeds_padded.append(torch.cat((
                    cur_embed,
                    torch.zeros((max_len - cur_len, cur_embed.shape[1]), dtype=cur_embed.dtype, device=cur_embed.device)
        ...
        input_embeds_tensor = torch.stack(input_embeds_padded, dim=0)
        return input_embeds_tensor, labels_padded, attention_mask_padded, position_ids_padded

    def _assemble_text_and_feature_embeddings(self,
                                              input_ids,
                                              labels,
                                              attention_mask,
                                              position_ids,
                                              ...
                                              new_frame_num):
        """
        Build input embeddings by interleaving text token embeddings with per-sample feature tokens
        around `IMAGE_TOKEN_INDEX`, then pad/stack and reconcile optional inputs.

        Returns: (input_embeds_tensor, labels_out, attention_mask_out, position_ids_out)
        """
        # Keep originals to restore None semantics and dtypes later
        _labels = labels
        _position_ids = position_ids
        _attention_mask = attention_mask

        # Default missing tensors and normalize dtypes
        ...
        # Remove padding based on the attention mask to get per-sample 1D sequences
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            # Count modality insertions
            num_images = (cur_input_ids == IMAGE_TOKEN_INDEX).sum()
            if num_images == 0:
                # No image token: pure text path (keep shapes consistent)
            ...
            # Split sequences by image tokens, embed text chunks, and interleave with features
            cur_labels = labels[batch_idx]
            image_token_indices = [-1] + torch.where(cur_input_ids == IMAGE_TOKEN_INDEX)[0].tolist() + [cur_input_ids.shape[0]]
            cur_input_ids_noim = [cur_input_ids[start + 1:end] for start, end in zip(image_token_indices[:-1], image_token_indices[1:])]
            cur_labels_noim = [cur_labels[start + 1:end] for start, end in zip(image_token_indices[:-1], image_token_indices[1:])]
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    # Insert corresponding feature span after each image token
                    cur_image_features = video_features[cur_image_idx]
            ...
            # Move to correct device and finalize tensors for this sample
        ...
        # Optional truncation for safety with long sequences
        ...
        input_embeds_tensor, labels_padded, attention_mask_padded, position_ids_padded = self._pad_and_stack_new_sequences(
            new_input_embeds,
            new_labels,
            attention_mask,
            position_ids,
        ...
        # Restore None semantics and original dtypes where applicable
        if _labels is None:
            labels_out = None
        else:
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if images is not None and (-1 not in image_sizes):
            image_features, video_idx_in_batch, image_features_before_mlp, scaled_shape = self.prepare_image_features(images, image_sizes, modalities, return_feat_before_mlp=True) # [torch.Size([32, 196, 896]), torch.Size([32, 196, 896])]
        ...
        # import ipdb
        # ipdb.set_trace()
        # get other modality feature (The fast path feature)
        ...
        if images is not None:
            if "3d_feature" in modalities[0]:
                video_features = torch.stack(video_features, dim=0)
                video_features = video_features.view(-1, video_features.shape[2], video_features.shape[3], video_features.shape[-1])
                video_features = self.image_shape_alignment(video_features, scaled_shape)
                video_features = list(torch.split(video_features, tuple(feat_frame_nums), dim=0))
        ...
        if video_features is not None:
            if feat_combine_method == 'concat':
                for batch_idx, (curr_video_feat, curr_image_feat) in enumerate(zip(video_features, image_features)):
                    ...
                    curr_video_feat = torch.cat([curr_image_feat, curr_video_feat], dim=0)
                    video_features[batch_idx] = curr_video_feat
                new_frame_num = [ele.shape[0] for ele in video_features]
            #TODO: finish the interleave version
                ...
                raise ValueError(f"Unexpected feat_combine_method: {feat_combine_method}")
